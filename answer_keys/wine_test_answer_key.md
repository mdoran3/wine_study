# Wine Test — Answer Key

All 32 possible questions (16 wines × 2 question types).

---

## Region Questions

| Wine | Region |
|---|---|
| Tarima | Alicante |
| Shaya | Rueda |
| Polvorete | Bierzo |
| Inazio Urruzola | Getariako Txakolina |
| Hacienda Arinzano Blanco | Navarra |
| Mar de Frades | Rias Baixas |
| Pazo Barrantes | Rias Baixas |
| El Coto "Crianza" | Rioja |
| Finca Resalso | Ribera del Duero |
| Juan Gil Silver | Jumilla |
| Termes | Toro |
| Marqués de Murrieta "Reserva" | Rioja |
| Juan Gil Blue | Jumilla |
| Clio | Jumilla |
| Emilio Moro | Ribera del Duero |
| Hacienda Arinzano Rosé | Navarra |

---

## Grape Questions

| Wine | Grapes |
|---|---|
| Tarima | Moscatell, Merseguera |
| Shaya | Verdejo |
| Polvorete | Godello |
| Inazio Urruzola | Hondarribi Zuri |
| Hacienda Arinzano Blanco | Chardonnay |
| Mar de Frades | Albariño |
| Pazo Barrantes | Albariño |
| El Coto "Crianza" | Tempranillo |
| Finca Resalso | Tempranillo |
| Juan Gil Silver | Monastrell |
| Termes | Tempranillo |
| Marqués de Murrieta "Reserva" | Tempranillo, Mazuelo, Graciano, Garnacha |
| Juan Gil Blue | Monastrell, Cabernet, Syrah |
| Clio | Monastrell, Cabernet |
| Emilio Moro | Tempranillo |
| Hacienda Arinzano Rosé | Tempranillo |

---

## Quick Reference by Type

### Whites
| Wine | Region | Grapes |
|---|---|---|
| Tarima | Alicante | Moscatell, Merseguera |
| Shaya | Rueda | Verdejo |
| Polvorete | Bierzo | Godello |
| Inazio Urruzola | Getariako Txakolina | Hondarribi Zuri |
| Hacienda Arinzano Blanco | Navarra | Chardonnay |
| Mar de Frades | Rias Baixas | Albariño |
| Pazo Barrantes | Rias Baixas | Albariño |

### Reds
| Wine | Region | Grapes |
|---|---|---|
| El Coto "Crianza" | Rioja | Tempranillo |
| Finca Resalso | Ribera del Duero | Tempranillo |
| Juan Gil Silver | Jumilla | Monastrell |
| Termes | Toro | Tempranillo |
| Marqués de Murrieta "Reserva" | Rioja | Tempranillo, Mazuelo, Graciano, Garnacha |
| Juan Gil Blue | Jumilla | Monastrell, Cabernet, Syrah |
| Clio | Jumilla | Monastrell, Cabernet |
| Emilio Moro | Ribera del Duero | Tempranillo |

### Rosé
| Wine | Region | Grapes |
|---|---|---|
| Hacienda Arinzano Rosé | Navarra | Tempranillo |
